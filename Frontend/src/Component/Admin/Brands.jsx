...
const Brands = () => {
    ...
    useEffect(() => {
        ...
        const fetchItems = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:8080/api/v1/admin/items/getBrands`, {
                    headers: {
    ...
    const handleSearch = () => {
        ...
        const filteredItems = items.filter(item => {
            const cleanedTitle = item.brand.toLowerCase().replace(/[^\w\s]/gi, ''); // Remove symbols
    ...
    const handleUpdate = async (itemId) => {
        try {
          const response = await axios.put(`http://localhost:8080/api/v1/admin/items/updateBrand/${itemId}`,{
            brand: update
          }, {
            headers: {
              'Content-Type': 'application/json',
      ...
      const handleAdd = async () => {
        try {
          const response = await axios.post(`http://localhost:8080/api/v1/admin/items/addBrands`,{
            brand: newAdd, 
          }, {
            headers: {
              'Content-Type': 'application/json',
      ...
      const handleDelete = async (itemId) => {
        try {
          const response = await axios.delete(
            `http://localhost:8080/api/v1/admin/items/deleteBrand/${itemId}`,
            {
              data: {
                reason: reason,
                adminId: adminId
  ...
  const textStyle = {
    ...
    return (
        <>
           ...
           <div className='fs-2 text-center'><b>Brands</b></div>
            <div className='container'>
                    ...
                    <Modal
                        ...
                        style={modalStyle2}>
                            <div className='fs-1 text-info bg-dark text-center '>Attention</div>
                                <div className='fs-4 text-info bg-dark text-center my-3'>Add new Brand via input field and press "yes" button.</div>
                ...
                <div className='col-lg-2'><button className="btn m-2 my-sm-0 btn-outline-secondary mr-4" type="button" onClick={handleDeleteOnes} style={textStyle}>Delete Brands</button></div>
                                ...
                                <div className='row my-2 font-weight-bold text-dark'>    
                                    <div className='col-lg-12 col-md-6'>                                                                         
                                            <div className='col-lg-12 col-md-6  my-3 font-weight-bold text-dark'>
                                                    <div className="fs-4 text-center text-light">Brand Name: {item.brand}</div>                                                                          
                                        </div>        
                                    </div>
                                </div> 
                                <div className='row'>
